class Node:
    ...
    def fin_me(root,lst):
        if root == None:
            return
        Node.fin_me(root.left,lst)
        Node.fin_me(root.right,lst)
        lst.append(root.data)

    def rebar(root,val,dir):
        temp = root
        ...
        #find val sucess
        lst_ =[]
        Node.fin_me(temp,lst_)       
        if int(val) not in lst_:
            print(f"No {val} in this tree") 
            return 1
        if dir == "left":
            lst =sorted(lst_,reverse=True)            
            newTree  =  Node(lst[0])
            temp_new = newTree
            for i in lst[1:]:
